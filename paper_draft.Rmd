...
wave exposure is regarded as another important abiotic factor in shaping kelp communities. Add section here....
...
community_df <- read.csv(file = "data/community.csv", sep = ";", check.names = FALSE)
...
fauna_sum <- fauna_lng %>% 
  ...
  summarise(mean_biomass = mean(biomass, na.rm = T),
            min_biomass = min(biomass, na.rm = T),
            max_biomass = max(biomass, na.rm = T),
            range_biomass = range(biomass, na.rm = T)[2]-range(biomass, na.rm = T)[1],
            sd_biomass = sd(biomass, na.rm = T)) %>% 
  ungroup() 

fauna_sum <- melt(fauna_sum, id.vars = c("site", "species"), value.name = "value")

plot_fauna <- ggplot(data = fauna_sum, aes(x= species, y = mean_biomass, color = site)) 
plot_fauna <- plot_fauna + geom_point()
plot_fauna <- plot_fauna + theme(axis.text.x = element_text(angle = 90, hjust = 0, vjust = 0))
plot_fauna <- plot_fauna + coord_flip()
...
flora_sum <- melt(flora_sum, id.vars = c("site", "species"), value.name = "mean_biomass")


plot_flora <- ggplot(data = flora_sum, aes(x= species, y = mean_biomass, color = site)) 
plot_flora <- plot_flora + geom_point()
plot_flora <- plot_flora + theme(axis.text.x = element_text(angle = 90, hjust = 0, vjust = 0))
plot_flora <- plot_flora + coord_flip()
