...
```{r diversity graphs script, message=FALSE, warning=FALSE, paged.print=FALSE}
...
plot_fauna <- ggplot(data = fauna_lng, aes(x=site, y=biomass, colour = species)) +                             stat_summary(geom = "pointrange", data = fauna_lng) + 
              theme(axis.text.x=element_text(angle = 90, hjust = 1, vjust = 0.3)) +
              theme(panel.background = element_rect(fill = 'grey', colour = 'grey')) +
              ylab("") + 
              xlab("") +
              coord_flip()
grid.arrange(plot_flora, plot_fauna)
